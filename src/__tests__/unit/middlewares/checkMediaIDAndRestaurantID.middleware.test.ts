...
// unit testing
describe('checkMediaIDLinkedToRestaurantIDMiddleware', () => {
  afterEach(() => {
    jest.resetAllMocks();
  });
  describe('mediIDLinkedToRestaurantIDMiddleware', () => {
    const MEDIA_ID = 10;
    const RESTAURANT_ID = 1;
    it('should successfully validate mediaID linked to restaurant ', async () => {
      ...
      await checkMediaIDLinkedToRestaurantIDMiddleware(mReq as Request, mRes as Response, mNext as NextFunction);
    });
    it('should throw 400 HTTP exception if restaurantID not provided in body request', async () => {
    ...
    it('should throw 400 HTTP exception if mediaID not provided in body request', async () => {
      // mock a request needed by controller
      const mReq: Partial<Request> = {
        headers: {
          authorization: 'token',
    ...
    it('should throw 401 HTTP exception if mediaID not provided in body request', async () => {
      // mock a request and response needed by controller
      const mReq: Partial<Request> = {
        params: {
          mediaID: `${MEDIA_ID}`,
